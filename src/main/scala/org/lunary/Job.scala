...
class Job(implicit config: AreaConfig, backend: SttpBackend[Future, Nothing], ec: ExecutionContext) {

  private val rand = new SecureRandom

  def request(category: String): Future[Either[Throwable, String]] = {
    ...
    val request = sttp
      ...
      .body("free" -> "", "rarelity" -> "", "battle_type" -> "", "card_type" -> "",
        "category_exp" -> category, "title_name" -> "",
        "btn_search.x"-> s"${rand.nextInt(96) + 2}", "btn_search.y" -> s"${rand.nextInt(45) + 2}",
        "btn_searc" -> "検索")

    val postRequest = request.post(uri"$searchUrl").response(asString("utf-8"))


    val start = System.currentTimeMillis()
    postRequest.send().map {resp =>
      resp.body match {
        case Left(e) => Left(new RuntimeException(e))
        case Right(b) =>
